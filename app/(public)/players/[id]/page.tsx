...
export default async function PlayerProfilePage({
  ...
  const { data: aliases } = (await supabase
    ...
    .eq('player_id', id)
    .order('alias_display')) as {
    data: Pick<PlayerAlias, 'id' | 'alias_display'>[] | null
    error: unknown
  }
  ...
  const isOwnProfile =
    !!player.profile_id && player.profile_id === session.userId
  ...
  if (isOwnProfile && gameIds.length > 0) {
    ...
    const { data: games } = (await supabase
      ...
      data:
        | Pick<Game, 'id' | 'date' | 'location' | 'score_a' | 'score_b'>[]
        | null
      error: unknown
    }
    ...
    const ratingByGame = new Map(
      (ratings ?? []).map(r => [r.game_id, r.rating]),
    )
    ...
    <div className="container max-w-screen-md mx-auto px-4 py-8 space-y-8">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-fcda-navy">
            {player.display_name}
          </h1>
        </div>
        {(aliases ?? []).length > 0 && (
