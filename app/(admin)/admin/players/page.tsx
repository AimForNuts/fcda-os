...
export default async function PlayersPage() {
  ...
  const { data: players } = (await admin
    .from('players')
    .select(
      'id, sheet_name, shirt_number, current_rating, preferred_positions, profile_id',
    )
    .order('sheet_name')) as {
    data: Array<{
      id: string
  ...
  const aliasesByPlayer = new Map<
    string,
    Array<{ id: string; alias_display: string }>
  >()
  for (const a of allAliases) {
    if (!aliasesByPlayer.has(a.player_id)) aliasesByPlayer.set(a.player_id, [])
    aliasesByPlayer
      .get(a.player_id)!
      .push({ id: a.id, alias_display: a.alias_display })
  }
  ...
  const rows: PlayerRow[] = playerList.map(p => ({
    ...
    profile_name: p.profile_id
      ? (profileNames.get(p.profile_id) ?? null)
      : null,
